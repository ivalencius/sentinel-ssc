...
# Working Directory
wd_root <- "/Users/ilanvalencius/Documents/River-Sed-Manuscript"
setwd(wd_root)
# Exports folder
...
df <- read.csv('/Users/ilanvalencius/Documents/River-Sed-Manuscript/figure-data/RATING_regression/holdout_data.csv')
...
sample_hist <- ggplot(rating_samples, aes(log10(SSC_MGL), fill="Rating Curve Derived")) +
  geom_histogram(col="black", bins=40, alpha=0.8) +
...
rating_curve <- ggplot(insitu_rating_data, aes(x = LOG10_Q_M3S, y = LOG10_SSC_FLUX_MTYR)) +
  ...
  labs(
    ...
    title = paste0("USGS Station ", station_sel, " (R2=", ex_R2,")")) +
  theme(text = element_text(family = "JetBrains Mono NL"))
...
# 25 x 18 aspect ratio roughly
ggsave(filename = paste0(wd_exports, "fig2.png"), height = 10, width = 13.8, units = "in")
...
ggsave(filename = paste0(wd_exports, "regression-residuals.png"), height = 12, width = 12, units = "in")
...
holdout <- df
# holdout$BIAS <- abs(log10(10^holdout$PRED_LOG10_SSC_MGL/10^holdout$LOG10_SSC_MGL))-1)
# One invalid bias number - remove
# holdout <- holdout %>% filter(is.finite(BIAS))

holdout_bias <- ggplot(holdout, aes(
  y = (10^abs(log10(10^PRED_LOG10_SSC_MGL/10^LOG10_SSC_MGL)))-1, 
  x="")) + 
...
ggarrange(holdout_bias, cluster_bias, ncol = 2, widths=c(1,3))

ggsave(filename=paste0(wd_exports, "cluster-bias.png"), height = 7, width = 10, units = "in")

###################### Fig 6 - Chattahoochee ###################################
# Majority cluster
chatM <- read.csv("/Users/ilanvalencius/Documents/River-Sed-Manuscript/figure-data/CHAT4.csv")
# Variable cluster
chatV <- read.csv("/Users/ilanvalencius/Documents/River-Sed-Manuscript/figure-data/CHATVAR.csv")
...
ggarrange(v, m, ncol = 2, widths=c(1,1))
...
# USGS data
usgs <- read.csv("/Users/ilanvalencius/Documents/River-Sed-Manuscript/figure-data/PROCESSED_CHATTAHOOCHEE_SSC.csv")
usgs$dist_group <- floor(usgs$dist_downstream_km/10) * 10
# Filter from data 2000 - now
usgs <- usgs %>% filter(sample_dt >= 1990)
# Load dams
dams <- read.csv("/Users/ilanvalencius/Documents/River-Sed-Manuscript/figure-data/ERDC_CHATTAHOOCHEE_DAMS.csv")
...
ggplot(chatM, aes(group=dist_group, x=dist_group, y=PRED_SSC_MGL)) +
  ...
  geom_boxplot(data=usgs, aes(y=SSC_mgL, color='USGS'), width=20, outlier.shape = NA) +
  geom_vline(xintercept = dams$distance_km, color="red", linewidth=2) +
  coord_cartesian(ylim = c(0, 150)) +
  geom_point(data=active, aes(x=dist_downstream_km, y=0, group=NA), fill="purple", size=10, shape=24) +
  theme_bw() +
